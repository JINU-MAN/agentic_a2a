...
def scrape_papers_with_mcp(query: str) -> str:
    ...
    logger.debug("scrape_papers_with_mcp: query=%s", safe_query)
    try:
        raw = call_mcp_tool(
        ...
        logger.debug("scrape_papers_with_mcp completed: query=%s result_count=%d", safe_query, len(items))
        return result_text
    except Exception as e:
        logger.exception("scrape_papers_with_mcp failed: query=%s", safe_query)
        raise
...
def fetch_external_paper_with_mcp(
    ...
    logger.debug(
        "fetch_external_paper_with_mcp: reference=%s url=%s doi=%s arxiv_id=%s",
        safe_reference,
        safe_url,
        safe_doi,
        safe_arxiv_id,
    ...
    try:
        ...
        logger.debug("fetch_external_paper_with_mcp completed: ok=%s", ok)
        return result_text
    except Exception as e:
        logger.exception("fetch_external_paper_with_mcp failed: reference=%s", safe_reference)
        raise
...
def load_paper_memory_with_mcp(
    ...
    logger.debug(
        "load_paper_memory_with_mcp: query=%s workflow_id=%s load_mode=%s",
        safe_query,
        safe_workflow_id,
        safe_load_mode,
    )
    ...
    try:
        ...
        logger.debug(
            "load_paper_memory_with_mcp completed: workflow_id=%s loaded_count=%d",
            safe_workflow_id,
            len(loaded_papers),
        )
        return result_text
    except Exception as e:
        logger.exception("load_paper_memory_with_mcp failed: query=%s", safe_query)
        raise
...
def expand_paper_memory_with_mcp(
    ...
    safe_max_chars = max(2000, min(int(max_chars_per_paper), MAX_MEMORY_CHARS_PER_PAPER))
    logger.debug(
        "expand_paper_memory_with_mcp: workflow_id=%s question=%s",
        safe_workflow_id,
        safe_question,
    )
    try:
        ...
        logger.debug(
            "expand_paper_memory_with_mcp completed: workflow_id=%s expanded_count=%d",
            safe_workflow_id,
            len(expanded),
        )
        return result_text
    except Exception as e:
        logger.exception("expand_paper_memory_with_mcp failed: workflow_id=%s", safe_workflow_id)
        raise
...
def query_paper_memory(
    ...
    logger.debug(
        "query_paper_memory: workflow_id=%s question=%s",
        safe_workflow_id,
        safe_question,
    )
    try:
        ...
        logger.debug(
            "query_paper_memory completed: workflow_id=%s snippet_count=%d",
            safe_workflow_id,
            len(picked),
        )
        return result_text
    except Exception as e:
        logger.exception("query_paper_memory failed: workflow_id=%s", safe_workflow_id)
        raise
